...
<?php if ($_['internalOnly'] === false): ?>
	...
	<div id="fileSharingSettings" class="section">
		...
		<p>
			<input type="checkbox" name="outgoing_server2server_share_enabled"
				   id="outgoingServer2serverShareEnabled" class="checkbox"
				   value="1" <?php if ($_['outgoingServer2serverShareEnabled']) {
				print_unescaped('checked="checked"');
			} ?> />
			<label for="outgoingServer2serverShareEnabled">
				<i><?php p($l->t('Allow users on this server to send shares to other servers (this option also allows WebDAV access to public shares)')); ?></i>
			</label>

		</p>
		<p>
			<input type="checkbox" name="incoming_server2server_share_enabled"
				   id="incomingServer2serverShareEnabled" class="checkbox"
				   value="1" <?php if ($_['incomingServer2serverShareEnabled']) {
				print_unescaped('checked="checked"');
			} ?> />
			<label for="incomingServer2serverShareEnabled">
				<i><?php p($l->t('Allow users on this server to receive shares from other servers'));?></i>
			</label><br/>
		</p>
		<?php if ($_['federatedGroupSharingSupported']): ?>
			<p>
				<input type="checkbox"
					   name="outgoing_server2server_group_share_enabled"
					   id="outgoingServer2serverGroupShareEnabled"
					   class="checkbox"
			...
			<p>
				<input type="checkbox"
					   name="incoming_server2server_group_share_enabled"
					   id="incomingServer2serverGroupShareEnabled"
					   class="checkbox"
		...
		<?php endif; ?>
		<p>
			<input type="checkbox" name="lookupServerEnabled"
				   id="lookupServerEnabled" class="checkbox"
		...
		<p>
			<input type="checkbox" name="lookupServerUploadEnabled"
				   id="lookupServerUploadEnabled" class="checkbox"
